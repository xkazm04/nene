...
async def research_statement(request: ResearchRequestAPI) -> EnhancedLLMResearchResponse:
    ...
    try:
        ...
        # Process speaker profile (non-blocking)
        profile_id = None
        ...
        enhanced_result = EnhancedLLMResearchResponse(
            ...
            experts=result.experts,  # Add this line
...
async def search_research_results(
    ...
    """
    try:
        logger.info(f"Searching research results: search='{search}', status='{status}', country='{country}', category='{category}'")
