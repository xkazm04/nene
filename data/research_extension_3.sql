...
-- FIXED VERSION: Resolves GIN index issues with TEXT fields
...
ADD COLUMN IF NOT EXISTS key_findings TEXT[] DEFAULT '{}',  -- Use TEXT array with default empty array
...
-- JSONB GIN index (this works perfectly)
CREATE INDEX IF NOT EXISTS idx_research_results_research_metadata 
ON research_results USING gin(research_metadata);

-- GIN indexes for TEXT arrays (these work because arrays have proper operator classes)
CREATE INDEX IF NOT EXISTS idx_research_results_key_findings 
ON research_results USING gin(key_findings);

CREATE INDEX IF NOT EXISTS idx_research_results_llm_findings 
ON research_results USING gin(llm_findings);

CREATE INDEX IF NOT EXISTS idx_research_results_web_findings 
ON research_results USING gin(web_findings);

CREATE INDEX IF NOT EXISTS idx_research_results_resource_findings 
ON research_results USING gin(resource_findings);
